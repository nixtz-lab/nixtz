 ...
 *
 * MODIFIED (LATEST - GEMINI):
 * - Added hardcoded 'CASH' suggestion to fetchTickerSuggestions().
 * - Added hardcoded 'PROPERTY' suggestion to fetchTickerSuggestions().
 * - Updated selectPortfolioSuggestion() to auto-set the Asset Class to 
 * 'Cash' or 'REIT' when 'CASH' or 'PROPERTY' is selected.
...
// --- START: SELECT PORTFOLIO SUGGESTION (MODIFIED) ---
function selectPortfolioSuggestion(ticker, description = null) {
    ...
    // ****** MODIFIED: Auto-set Asset Class for CASH and PROPERTY ******
    const upperTicker = ticker.toUpperCase();
    if (upperTicker === 'CASH') {
        assetClassSelect.value = 'Cash'; // Matches <option value="Cash">
    } else if (upperTicker === 'PROPERTY') {
        assetClassSelect.value = 'REIT'; // Matches <option value="REIT">
    } else {
        // Default to Equity for all other tickers, or keep existing value if not one of the hardcoded ones
...
// --- END: SELECT PORTFOLIO SUGGESTION (MODIFIED) ---

// --- START: FETCH TICKER SUGGESTIONS (MODIFIED) ---
async function fetchTickerSuggestions(query) {
    if (query.length < 2) return [];

    // ****** MODIFIED: Hardcoded suggestions for CASH and PROPERTY ******
    const upperQuery = query.toUpperCase();
    const hardcodedSuggestions = [];
    
    // Add CASH
    if ('CASH'.startsWith(upperQuery)) {
         hardcodedSuggestions.push({ 
            ticker: 'CASH', 
            description: 'CASH - Cash/Money Market Account' 
        });
    }
    
    // Add PROPERTY
    if ('PROPERTY'.startsWith(upperQuery)) {
         hardcodedSuggestions.push({ 
            ticker: 'PROPERTY', 
            description: 'PROPERTY - Manual Real Estate Holding/REIT' 
        });
    }
    // ******************************************************************
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return hardcodedSuggestions; // Return hardcoded suggestions on API failure
        }
        const result = await response.json();
        
        if (result.success && Array.isArray(result.data)) {
            const apiResults = result.data.map(item => ({ 
                ticker: item.ticker, 
                description: `${item.ticker} - ${item.name}` 
            }));
            
            // Combine hardcoded and API results
            const allResults = [...hardcodedSuggestions, ...apiResults];
            // Filter unique results by ticker
            const uniqueResults = allResults.filter((v, i, a) => a.findIndex(t => (t.ticker === v.ticker)) === i);
            return uniqueResults;
        }
        return hardcodedSuggestions;
    } catch (error) {
        console.error("Network error fetching ticker suggestions:", error);
        return hardcodedSuggestions; // Return hardcoded suggestions on network error
    }
}
// --- END: FETCH TICKER SUGGESTIONS (MODIFIED) ---
...
async function fetchLiveData(holdings) {
    const tickers = [...new Set(holdings.map(h => h.ticker).filter(t => t !== 'CASH' && t !== 'PROPERTY'))];
    const tickerString = tickers.join(',');

    const liveData = { 
        'CASH': { price: 1.0, dividend_yield: 0.0000 },
        // PROPERTY is a manual entry, so treat its price as its buy_price/cost_basis
        'PROPERTY': { price: 1.0, dividend_yield: 0.0000 } 
    };
...
function processPortfolioData(holdings, liveData) {
    ...
    holdings.forEach(h => {
        ...
        if (h.ticker === 'CASH' || h.ticker === 'PROPERTY') {
            currentPrice = h.buy_price; // Use buy price for manual assets
            priceIsLive = true;
        } else {
...
function drawAssetAllocationChart(allocationData) {
    ...
    const chartColors = labels.map(label => {
        ...
        if (label.includes('REIT')) return '#9966FF'; // Added REIT color
        return '#9966FF'; 
    });
...
function drawStockAllocationChart(allocationData, totalPortfolioValue) {
    ...
    // Filter out manual tickers (CASH, PROPERTY) from the stock allocation chart
    const filteredData = Object.entries(data)
        .filter(([ticker]) => ticker !== 'CASH' && ticker !== 'PROPERTY');
    
    const filteredLabels = filteredData.map(([ticker]) => ticker);
    const filteredValues = filteredData.map(([, value]) => value);
    ...
    stockAllocationChartInstance = new Chart(document.getElementById('stock-allocation-chart'), {
        ...
        data: {
            ...
            datasets: [{
                data: filteredValues,
                backgroundColor: chartColors.slice(0, filteredLabels.length),
                borderColor: '#121212',
                borderWidth: 2,
            }]
        },
...
function drawMyPositionsValueChart(allocationData) {
    ...
    // Filter out CASH and PROPERTY positions, then sort
    const sortedData = Object.entries(data)
        .filter(([ticker, value]) => value > 0 && ticker !== 'CASH' && ticker !== 'PROPERTY')
        .sort((a, b) => b[1] - a[1]); 
...
$(document).ready(function() {
    ...
    $('#ticker-input').on('keyup', async function() {
        ...
        // Use the modified fetchTickerSuggestions
        const results = await fetchTickerSuggestions(query);
        
        if (results.length > 0) {
            results.forEach(item => {
                // Ensure description handles single quotes for the onclick function
                ...
                const suggestionItem = $('<div>')
                    ...
                    .attr('onclick', `selectPortfolioSuggestion('${item.ticker}', '${safeDescription}')`); 
