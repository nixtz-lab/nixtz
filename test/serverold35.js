...
// *** NEW: In-memory cache for watchlist ***
...
// *** ROLLED BACK TO ORIGINAL API HOST (apidojo) ***
...
app.get('/api/stock-snapshot/:ticker', async (req, res) => {
    ...
    try {
        ...
        const rawFiftyTwoWeekHigh = quote.summaryDetail?.fiftyTwoWeekHigh?.raw;
        data.fiftyTwoWeekHigh = isNumber(rawFiftyTwoWeekHigh) ? `$${rawFiftyTwoWeekHigh.toFixed(2)}` : "N/A"; // Matches dashboard
        ...
        // MODIFIED ATH LOGIC: Strict check for All-Time High only
...
app.get('/api/stock-yahu-snapshot/:ticker', async (req, res) => {
    ...
    try {
        ...
        // *** ROLLED BACK: This logic is unreliable and is now replaced by SimFin in the historicals route ***
        data.netAssets = "N/A"; 
        // *** END ROLLED BACK ***
        ...
        console.log(`[${new Date().toISOString()}] Successfully processed Yahu data for ${ticker} (excluding unreliable Yahu Net Assets). Sending FLAT response.`);
...
app.post('/api/stock/batch-details', authMiddleware, async (req, res) => {
    ...
    // API Rate Limit Settings (Rolled back to safe 5 req/sec)
    const CHUNK_SIZE = 5; 
    const DELAY_MS = 1000; 
...
app.post('/api/user/watchlist/add', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        // *** NEW: Clear cache for this user ***
        watchlistCache.delete(req.user.id);
        console.log(`Watchlist cache cleared for user: ${req.user.id}`);
