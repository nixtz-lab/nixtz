...
// LIVE ALPHA VANTAGE API CONFIGURATION (Using your key)
const ALPHA_VANTAGE_API_KEY = '395e634198msh3ce064e80779cb8p161601jsn07aeb51f16ae';
const ALPHA_VANTAGE_HOST = 'alpha-vantage.p.rapidapi.com';
const ALPHA_VANTAGE_URL = `https://${ALPHA_VANTAGE_HOST}/query`;
...
async function fetchExchangeRates(callback) {
    ...
    if (!ALPHA_VANTAGE_API_KEY || ALPHA_VANTAGE_API_KEY.includes('YOUR_KEY')) {
         showMessage("Currency API Key missing. Cannot fetch live rates.", "error");
         if (callback) callback();
         return;
    }
    ...
    const url = `${ALPHA_VANTAGE_URL}?function=CURRENCY_EXCHANGE_RATE&from_currency=${baseCurrency}&to_currency=${currentCurrency}`;
    
    try {
        const response = await fetch(url, { headers });
        if (!response.ok) {
            const errorText = await response.text();
            throw new Error(`HTTP error ${response.status}. Check API Key or endpoint.`);
        }
        ...
        if (data[exchangeRateField] && data[exchangeRateField][rateKey]) {
            const rate = parseFloat(data[exchangeRateField][rateKey]);
            
            if (isNaN(rate) || rate <= 0) {
                 throw new Error(`Invalid rate received from API: ${data[exchangeRateField][rateKey]}`);
            }
            ...
            showMessage(`Live rate for ${currentCurrency} updated to ${rate.toFixed(4)}!`, 'success');
             ...
             throw new Error("Could not parse rate response from API.");
        }
