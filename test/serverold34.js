...
app.put('/api/admin/membership-config/:level', authMiddleware, adminAuthMiddleware, async (req, res) => {
    ...
    try {
        ...
        // This ensures existing users get the updated permissions immediately
...
app.get('/api/stock/historicals/:ticker', async (req, res) => {
    ...
    try {
        ...
        let countNetIncome = 0, countFCF = 0, countEPS = 0, countRevPerShare = 0, countCFPerShare = 0;
        ...
        if (countNetIncome > 0) {
            ...
            historicalAverages.avgProfitMargin5Yr = `${((sumProfitMargin / countNetIncome) * 100).toFixed(2)}%`;
        }
        if (countFCF > 0) {
...
app.get('/api/stock-valuation-snapshot/:ticker', async (req, res) => {
    ...
    const apiKeyToUse = YAHU_RAPIDAPI_KEY;
    if (!apiKeyToUse) {
        console.error(`[${new Date().toISOString()}] FATAL: YAHU_RAPIDAPI_KEY missing! Check Coolify env vars.`);
        return res.status(500).json({ success: false, message: "Server configuration error: API key missing." });
    }
...
app.post('/api/admin/stock-rating', authMiddleware, adminAuthMiddleware, async (req, res) => {
    ...
    try {
        const updatedRating = await TmtStockRating.findOneAndUpdate(
            ...
            // MODIFICATION: Add 'rank' and 'targetPrice' to the data being saved
            { 
...
// --- START: MODIFIED WATCHLIST ROUTE WITH CHUNKING ---
/**
 * @route   POST /api/stock/batch-details
 * @desc    Get quote details for multiple tickers (for watchlist)
 * @desc    Uses CHUNKING to respect API rate limits (5/sec)
 * @access  Private
 */
app.post('/api/stock/batch-details', authMiddleware, async (req, res) => {
    ...
    let allResults = []; // To store results from all chunks
    ...
    try {
        ...
        for (let i = 0; i < tickers.length; i += CHUNK_SIZE) {
            ...
            // Wait for all API calls in this chunk to settle
            const chunkResponses = await Promise.allSettled(chunkPromises);
            allResults.push(...chunkResponses); // Add results to the main array

            // If this is not the last chunk, wait for 1 second
        ...
        // Now process allResults, which contains all promise states
        const formattedData = allResults.map((response, index) => {
            const ticker = tickers[index]; // Get original ticker by index
            ...
            if (response.status === 'fulfilled' && response.value.data?.quoteSummary?.result?.[0]) {
                ...
                if (quote) {
                    ...
                    if (change > 0) changeClass = 'text-tmt-green'; // These are custom colors, but JS will use them
                    if (change < 0) changeClass = 'text-tmt-red';   // Your JS (watchlist.js) will override this
                    ...
                    return {
                        ...
                        changeClass: changeClass // watchlist.js handles the final color
                    };
                }
            }
            
            // If the promise was rejected or data was invalid, return placeholder
        ...
        // This catch block is for catastrophic errors
...
// --- END ROUTE --- WATCHLIST Batch Stock Details Route  --

// ===================================================================
// 7. TMT DASHBOARD API ROUTES (NEWLY ADDED)
// ===================================================================
app.use('/api/tmt', tmtDashboardRoutes);
app.use('/api/projections', budgetPlannerRoutes);
// -------------------------------------------------------------------
// 8. OTHER EXISTING ROUTES (Copied from your uploaded file)
// -------------------------------------------------------------------
...
app.get('/api/user/watchlist', authMiddleware, async (req, res) => {
    try {
        // req.user is attached by authMiddleware and has the user's ID
        // We find the user but only select the 'watchlist' field
