...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchPendingUsers() {
        ...
        const pendingUsers = result || []; // Use result (which is result.data) or empty array
    ...
    async function fetchActiveUsers() {
        ...
        body.querySelectorAll('.membership-select').forEach(select => {
            select.addEventListener('change', async (e) => {
                ...
                const originalValue = e.target.querySelector('option[selected]').value; // Store original value
                ...
                if (result) {
                    ...
                    e.target.querySelector(`option[value="${newMembership}"]`).setAttribute('selected', true);
                     ...
                     e.target.value = originalValue; // Revert selection if API failed
                }
            });
        });
    }
    ...
    async function fetchTmtRatings() {
        ...
        // MODIFICATION: Use custom CSS classes from <style> tag in admin_panel.html
        listContainer.innerHTML = ratings.map(item => {
            const details = getRatingLabel(item.rating);
            // Use item.rank (from DB)
            ...
            return `
                <div class="tmt-rating-row"> <!-- USE CUSTOM CLASS -->
                    <div class="tmt-rating-left"> <!-- USE CUSTOM CLASS -->
                        <span class="font-bold text-lg text-white truncate">${item.ticker}</span>
                        ${rankDisplay}
                        ${targetPriceDisplay}
                    </div>
                    <div class="tmt-rating-right"> <!-- USE CUSTOM CLASS -->
                        <span class="text-sm font-semibold ${details.color}">${details.label} (${item.rating}/5)</span>
                        <!-- NEW DELETE BUTTON -->
                        <button data-ticker="${item.ticker}" class="delete-rating-btn tmt-delete-btn"> <!-- USE CUSTOM CLASS -->
        ...
        // Re-render icons for the new delete buttons
    ...
    function setupTmtRatingForm() {
        ...
        if (form) {
            // Remove previous listeners to prevent multiple saves
    ...
    async function handleSaveTmtRating(e) {
        ...
        // MODIFICATION: Get new rank and targetPrice values
        const rank = document.getElementById('tmt-rank-input').value.trim().toUpperCase();
        const targetPrice = document.getElementById('tmt-target-price-input').value; // Keep as string for API
        ...
        // MODIFICATION: Send all fields to the API
        ...
        if (result) {
            ...
            fetchTmtRatings(); // Refresh the list after saving
        }
    }

    // --- NEW: DELETE RATING HANDLER ---
    async function handleDeleteTmtRating(e) {
        ...
        if (result) {
            ...
            fetchTmtRatings(); // Refresh the list
        } else {
            // Re-enable button on failure
            e.currentTarget.disabled = false;
            if (icon) icon.classList.remove('animate-spin');
            // showMessage is already called by callApi on failure
        }
    }
