...
async function loadValuationData(ticker) {
    ...
    const getRobust5YrValue = (historical5YrValue, fallback3YrElementId) => {
        ...
        return document.getElementById(fallback3YrElementId).textContent;
    };
...
function searchStock() {
    ...
    if (input) {
        ...
        url.searchParams.set('ticker', input);
        window.history.pushState({}, '', url); // Update URL without reloading

        loadValuationData(input); // Reload data for the new ticker
    }
}
