...
document.addEventListener('DOMContentLoaded', () => {
        ...
        tabs.forEach(tab => {
            ...
            if (tabId === targetTab) {
                ...
                if (tabId === 'stock-ratings') { 
                    setupTmtRatingForm();
                    fetchTmtRatings(); // NEW: Load the list when the tab opens
                }
    ...
    async function fetchTmtRatings() {
        ...
        // This relies on the GET /api/admin/stock-ratings endpoint in server.js
        ...
        listContainer.innerHTML = ratings.map(item => {
            ...
            return `
                <div class="flex justify-between items-center bg-gray-800 p-3 rounded-lg border-l-4 border-tmt-primary/50">
                    <div class="flex items-center space-x-3">
                        <span class="font-bold text-lg text-white">${item.ticker}</span>
                        ${rankDisplay}
                    </div>
    ...
    async function handleSaveTmtRating(e) {
        ...
        // MODIFICATION: Get the new rank value
        ...
        // MODIFICATION: Send 'rank' in the API call
        const result = await callApi('/api/admin/stock-rating', 'POST', { ticker, rating, rank });
