...
function generateWeeklyRoster(staffProfiles, weekStartDate) {
    ...
    DAYS_FULL.forEach((day, dayIndex) => {
        ...
        deliveryDrivers.forEach((driver, index) => {
            ...
            if (driver.fixedDayOff === day) {
                ...
                if (otherDriver && otherDriver.fixedDayOff === day) {
                    ...
                    const shiftDetails = (driver.shiftPreference.includes('Morning')) 
        ...
        supervisors.forEach(sup => {
            ...
            else if (sup.shiftPreference === 'Morning' && supervisorDayOffs.get(supervisors.find(s => s.shiftPreference === 'Afternoon')?.name) === dayIndex) {
                supShift = { shiftId: 1, jobRole: 'C1 (Sup Cov)', timeRange: '08:00-20:00', color: ROLE_COLORS['Supervisor'] }; 
            } else if (sup.shiftPreference === 'Afternoon' && supervisorDayOffs.get(supervisors.find(s => s.shiftPreference === 'Morning')?.name) === dayIndex) {
                supShift = { shiftId: 2, jobRole: 'C1 (Sup Cov)', timeRange: '08:00-20:00', color: ROLE_COLORS['Supervisor'] };
            } else if (sup.shiftPreference !== 'Night' && supervisorDayOffs.get(supervisors.find(s => s.shiftPreference === 'Night')?.name) === dayIndex) {
                supShift = { shiftId: 3, jobRole: 'C1 (Sup Cov)', timeRange: '20:00-08:00', color: ROLE_COLORS['Supervisor'] };
            }
            else {
                const shiftId = sup.shiftPreference === 'Morning' ? 1 : sup.shiftPreference === 'Afternoon' ? 2 : 3;
        ...
        nightStaffPool.forEach(staff => {
            ...
            if (isDayOff) {
                weeklyRosterMap.get(staff.name).weeklySchedule[dayIndex].shifts.push({ shiftId: null, jobRole: 'Leave (Night)', timeRange: 'Full Day' });
            } else {
                weeklyRosterMap.get(staff.name).weeklySchedule[dayIndex].shifts.push({ shiftId: 3, jobRole: 'C2', timeRange: SHIFTS[3].time });
            }
        });
        
        let actualNightStaff = nightStaffPool.filter(s => nightStaffDaysOff.get(s.name) !== dayIndex).length;
        ...
        coveragePool.forEach(staff => {
            const alreadyScheduled = weeklyRosterMap.get(staff.name).weeklySchedule[dayIndex].shifts.length > 0;

            if (alreadyScheduled) return;
            
            const assignedDayOff = (staffCounter % 5) + 2;
            if (assignedDayOff === dayIndex) {
                 weeklyRosterMap.get(staff.name).weeklySchedule[dayIndex].shifts.push({ shiftId: null, jobRole: 'Leave', timeRange: 'Full Day' });
            } else {
                const shiftId = staff.shiftPreference === 'Morning' ? 1 : 2;
                const jobRole = staff.shiftPreference === 'Morning' ? 'C4' : 'C5';
                weeklyRosterMap.get(staff.name).weeklySchedule[dayIndex].shifts.push({ shiftId: shiftId, jobRole: jobRole, timeRange: SHIFTS[shiftId].time });
            }
            staffCounter++;
        });
