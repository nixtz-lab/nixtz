...
// API Hosts
const YFINANCE_HOST = process.env.YFINANCE_HOST; // <-- NOW PULLS FROM ENV VAR
...
const CRYPTO_FEAR_GREED_API_KEY = process.env.CRYPTO_FEAR_GREED_API_KEY; 
const CRYPTO_FEAR_GREED_API_HOST = process.env.CRYPTO_FEAR_GREED_API_HOST;
// --- END UPDATED F&G KEYS ---
...
router.get('/fear-and-greed', async (req, res) => {
    ...
    const url = `https://${FEAR_GREED_API_HOST}/index`;
    ...
    try {
        ...
        if (isNumber(score) && rating) {
            res.json({ 
             ...
             throw new Error('Invalid API response structure from Stock F&G API.');
        }
    } catch (err) {
        console.error('Stock Fear & Greed API Error:', err.message);
        let statusCode = 500;
        if (axios.isAxiosError(err) && err.response) {
            statusCode = err.response.status;
        }
        res.status(statusCode).json({ success: false, message: 'Failed to fetch Stock Fear & Greed data.' });
    }
});

router.get('/crypto-fear-and-greed', async (req, res) => {
    if (!CRYPTO_FEAR_GREED_API_KEY) {
        console.error("FATAL ERROR: CRYPTO_FEAR_GREED_API_KEY is not set in .env file.");
        return res.status(500).json({ success: false, message: "Server API key not configured." });
    }
    const url = `https://${CRYPTO_FEAR_GREED_API_HOST}/v1/fgi`;
    const options = {
        method: 'GET',
        headers: {
            'x-rapidapi-key': CRYPTO_FEAR_GREED_API_KEY,
            'x-rapidapi-host': CRYPTO_FEAR_GREED_API_HOST
        }
    };
    try {
        const response = await axios.get(url, options);
        ...
        if (data && data.value && data.value_classification) {
            res.json({ 
                success: true, 
            ...
            throw new Error('Invalid API response structure from Crypto F&G API.');
        }
    } catch (err) {
        console.error('Crypto Fear & Greed API Error:', err.message);
        let statusCode = 500;
        if (axios.isAxiosError(err) && err.response) {
            statusCode = err.response.status;
        }
        res.status(statusCode).json({ success: false, message: 'Failed to fetch Crypto Fear & Greed data.' });
    }
});
...
router.get('/tmt-top-5', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const formattedData = priceResponses.map((response, index) => {
            const dbInfo = topStocksDB[index]; 
            if (response.status === 'fulfilled' && response.value.data?.quoteSummary?.result?.[0]) {
                ...
                return {
                    ...
                    name: priceData?.longName || dbInfo.name,
...
// ===================================================================
// START OF UPDATED /top-decliners ROUTE
// ===================================================================
router.get('/top-decliners', authMiddleware, async (req, res) => {
    if (!YFINANCE_RAPIDAPI_KEY || !YFINANCE_HOST) {
        console.error("FATAL ERROR: YFINANCE_RAPIDAPI_KEY or YFINANCE_HOST is missing.");
        return res.status(503).json({ success: false, message: "Live data service unavailable." });
    }
    ...
    const options = {
        method: 'GET',
        url: `https://${YFINANCE_HOST}/api/market/get-day-losers`,
        params: { 
            count: '5', 
            offset: '0', 
            region: 'US',
    ...
    // --- END NEW URL AND PARAMS ---

    try {
        const response = await axios.request(options);
        
        // --- NEW RESPONSE PATH ---
        // This matches the JSON you sent: response.data.finance.result[0].quotes
        const quotes = response.data?.finance?.result?.[0]?.quotes;
        // --- END NEW RESPONSE PATH ---

        if (!quotes || !Array.isArray(quotes)) {
             throw new Error('Invalid data structure from YFinance API. Expected "quotes" array.');
        }
        
        // This mapping logic is still correct for the new API's quote structure
...
// ===================================================================
// END OF UPDATED /top-decliners ROUTE
// ===================================================================
