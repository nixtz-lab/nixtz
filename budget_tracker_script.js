...
async function addTransaction(e) {
    ...
    // FIX 2: Capture the date VALUE before any async processing
    ...
    // CRITICAL FIX 3: Use the captured date value to create the timestamp
    // Using T12:00:00.000Z ensures the UTC date is respected regardless of local timezone offset
    // This explicitly prevents the system clock from being used if the API call is slow.
    ...
    console.log(`Adding transaction: Desc='${finalDescription}', CatID='${categoryId}', Amount=${amountInCurrentCurrency} (${currentCurrency}), Base=${amountInBaseCurrency} (${baseCurrency}), Date=${dateValue}, Type='${type}'`);
    addButton.disabled = true; 
    ...
    try {
        ...
        // FIX 4: Reset the date input back to TODAY'S DATE after a successful save.
        if (dateInput) {
            ...
            console.log("Date input reset to today's date:", dateInput.value);
        }
